 ...
 * and width respectively. Ws is the source width. The calculations are
 * from "Constriction/Spreading Resistance Model for Electronic Packaging",
 * Seri LEE et al. ASME/JSME Thermal Engineering Conference vol. 4 1995.
 */

double getr(double k, double Wb, double Lb, double Ws)
{
	double theta, Bi, As, Ab, epsilon, tau, a, b, lamda, phi, psi, Rsprd;
	theta = k*(Lb/2)/(Wb*t_chip);
	As = Ws*t_chip;
	Ab = Wb*t_chip;
	a = sqrt(As/M_PI);
	b = sqrt(Ab/M_PI);
	...
	Rsprd = M_PI*M_PI*psi/(sqrt(M_PI)*k*a);
	return Rsprd;
}
